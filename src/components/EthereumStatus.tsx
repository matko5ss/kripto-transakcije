...
export default function EthereumStatus() {
  ...
  const fetchEthereumStatus = useCallback(async () => {
    try {
      ...
      setStatus(prev => ({
        previousBlock: prev.latestBlock,
        previousGasPrice: prev.gasPrice,
        previousEthPrice: prev.ethPrice,
        previousTransactionCount: prev.transactionCount,
        latestBlock: blockData.number || 0,
        gasPrice: gasData.gasPrice || '0',
        ethPrice: priceData.usd || '0',
  ...
  // Funkcija za provjeru je li vrijednost nova
  const isNew = (current: string | number, previous: string | number) => {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
        <div className={`p-4 rounded-lg ${
          isNew(status.latestBlock, status.previousBlock) 
          ...
          <p className="text-xl font-semibold">{status.latestBlock.toLocaleString()}</p>
        </div>
        
        <div className={`p-4 rounded-lg ${
          isNew(status.gasPrice, status.previousGasPrice) 
            ? 'bg-blue-50 dark:bg-blue-900/20 animate-pulse' 
            : ''
        }`}>
          <div className="flex justify-center mb-2">
          ...
          <p className="text-xl font-semibold">{formatGasPrice(status.gasPrice)}</p>
        </div>
        
        <div className={`p-4 rounded-lg ${
          isNew(status.ethPrice, status.previousEthPrice) 
            ? 'bg-blue-50 dark:bg-blue-900/20 animate-pulse' 
            : ''
        }`}>
          <div className="flex justify-center mb-2">
          ...
          <p className="text-xl font-semibold">${parseFloat(status.ethPrice).toFixed(2)}</p>
        </div>
        
        <div className={`p-4 rounded-lg ${
          isNew(status.transactionCount, status.previousTransactionCount) 
            ? 'bg-blue-50 dark:bg-blue-900/20 animate-pulse' 
            : ''
        }`}>
          <div className="flex justify-center mb-2">
          ...
          <p className="text-xl font-semibold">{status.transactionCount.toLocaleString()}</p>
        </div>
      </div>
    </div>
  );
